...
- [已完备] 锁定状态栏当前曲目信息卡片为 140px 宽度并平滑省略截断，杜绝控制按键移位 (2026-09-13):
  ...
  CDP 9222 实机自动化抓轨长文本注入测试：无论歌曲名是一字、四字还是数十字超长交响乐，播控组 left 坐标严格恒定在 254px（位移量严格为 0px，提交 3bfda2c）。
- [已完备] 将播放模式切换按键移至播放控制组右侧紧随下一曲排布 (2026-09-13):
  根据用户指令将原本孤立悬挂在最右侧的播放模式按键（.status-bar-btn-mode）迁移至左侧核心播放控制组（.status-bar-controls）内，紧随下一曲按键右侧；
  形成 [封面 + 歌曲信息 + 上一曲 + 播放暂停 + 下一曲 + 播放模式] 一气呵成、高内聚的左侧控制集群；
  彻底清理并移除底栏右侧扩展区（.status-bar-right-section 零残留），底栏右侧释放出连续 792px 纯净通透的液态高斯磨砂玻璃空间；
  CDP 9222 实机自动化抓轨断言单调递增坐标 (16px -> 86px -> 214px -> 260px -> 322px -> 368px)，全模式异步轮播切换验证全绿（提交 dd301c2）。
- [已完备] 将当前播放歌曲信息移至状态栏封面右侧，播放控制紧随其后 (2026-09-13):
  根据用户指令重构底部状态栏布局，将孤立悬挂在最右侧的当前歌曲元数据卡片（.status-bar-meta-block）移至 56px 专辑封面右侧（间距 14px）；
  排版重塑为自然左对齐（歌名居上、音质 SQ 徽标与艺人名居下）；
  播放控制按键组（.status-bar-controls: 上一曲/播放暂停/下一曲）紧随歌曲信息右侧（间距 28px），形成 [封面 + 歌曲信息 + 播放控制] 浑然一体的左侧核心操作集群；
  底栏中央释放出超 700px 的宽阔留白，右侧仅保留 1 枚 34px 极简播放模式按键，视觉秩序与通透感显著提升；
  CDP 9222 实机自动化抓轨 left: 16px -> 86px -> 214px 完美无缝递进（提交 b929bfe）。
...
- [已完备] 修复歌词滚轮滑动反向滞后与动量积压 Bug (2026-09-13):
  ...
  重构 handleWheel：以当前物理事件 e.deltaY 裁决真实方向、转向时立即清空异号残量实现 0 延迟即刻反转、离散大步进（>=24px）直接清空残量（1 刻度 = 1 行）、极值边界（next === base）即时灭尽动量防止撞墙积压、增加 150ms 停手自动复位定时器；
  CDP 9222 实机自动化验证：连续上划后首个下划刻度即刻响应 (+1)、连续下划后首个上划刻度即刻响应 (-1)、边界连续撞墙 15 次后反向即刻响应 (-1)，全量测试通过（提交 5880e19）。
- [已完备] 歌词旁圆弧型进度条与同心机芯联动，移除底栏旧进度条 (2026-09-13):
  在 LyricsOrbit.tsx 中彻底移除旧 r=360px 虚线弧与微细刻度线群；
  在同一同轴同心体系内（cx=0, cy=320, r=350px, 跨度 -52° ~ +52°，总行程 104°）构建优雅圆弧型音频播放进度条（底轨 + 薄荷绿高亮已播填充弧 + 5px 微晶游标滑珠 + 26px 隐形交互热区）；
  上方展示当前播放时间（mm:ss, 薄荷绿高亮纯文字），下方展示结束时间/总时长（mm:ss, 次级半透明近白纯文字），严格遵守纯文字去框原则；
  支持点击与拖拽寻道（基于 Math.atan2 极坐标映射，抬起时触发 seek 并施加 1200ms 寻道安全锁）；
  StatusBar 底部状态栏彻底移除原有直线进度条（.status-bar-progress-section）与相关计时状态，底栏留白开阔；
  圆弧进度条与星盘机芯完全作为单一刚体同轴联动，随设置项 X/Y 轴调节整体平移；CDP 9222 实机探测 hasOldBar=false、hasArcGroup=true，寻道精确响应（提交 3cb362b）。
