...
【当前开发进度（最新进展置顶，倒序排列）】
- [已完备] M2-2 悬浮胶囊播放队列管理 (QueueDrawer) 与曲库二态开关 (Toggle Switch):
  1. 播放队列抽屉 (QueueDrawer): 胶囊第 2 按键向右 360px 平滑展开收起，导轨锁定 56px 零抖动；
     全量呈现 MPD 实时队列，当前播放 `#6ee7b7` 翡翠高亮与微型跳动均衡器条，一键居中定位 (Locate)，
     单击即时切歌，单曲移除，一键清空队列，原生 HTML5 拖拽排序 (无外部依赖)，红心收藏持久化；
