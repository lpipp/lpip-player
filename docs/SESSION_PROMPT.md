...
- 已完成功能与架构：
  ...
  19. 液态玻璃进度条与时间指示器（StatusBar，左右两侧 01:14 / 04:08 时间指示器，强制 tabular-nums 等宽数字消除排版抖动；4px 纤细滑轨 + 10px 悬停发光滑块，支持点击/拖拽实时寻道）；
  20. 右侧精密机械表机芯齿轮与摆轮系统（MechanicalGear，高级制表纯线条蓝图工程风，全 fill:none 矢量线条，边缘微光漫射）：
      - 1号主齿轮：直径 500px，圆心距右边框 30px（外露 280px），垂直居中于可用区域（窗口顶至状态栏顶之间），60 齿线框圈，移除二级 36 齿内圈保持通透留白，顺时针旋转 180s/圈；
      - 2号次级齿轮：直径 180px，圆心在 1号上方 200px、距右边框 70px（外露 160px），24 齿精密齿圈，逆时针旋转 72s/圈（2.5倍传动比）；
      - 3号下级齿轮：直径 200px，圆心在 1号下方 280px、距右边框 0px（外露 100px 半圆），28 齿精密齿圈，逆时针旋转 84s/圈；
      - 飞轮（摆轮游丝系统）：直径 250px，圆心与 3号齿轮严格水平对齐，向左累计平移 70px（圆心距右边框 170px），外圈 16 颗微调配重砝码金螺钉、经典三臂轻量化轮辐（含双级平衡减重圈）、4.5 圈阿基米德精密储能螺旋游丝（随摆动同频呼吸收放）、因加百录防震宝石轴承；左右往复谐波摆动（cubic-bezier(0.45, 0.05, 0.55, 0.95) ±65° 周期 2.4s）；
      - 纵深透光层级：wrapper 设为 overflow: visible，3号齿轮与飞轮下部自然穿透进入 80px 磨砂状态栏下方，透过液态黑曜石玻璃呈现深邃的蓝宝石水晶夹板景深，杜绝生硬截断。
- 源码状态：
  - src/main: index.ts, config.ts, wallpaper.ts, mica.ts
  - src/renderer/src: App.tsx, styles/global.css, components/SidebarCapsule.tsx & .css, components/WallpaperLayer.tsx & .css, components/StatusBar.tsx & .css, components/MechanicalGear.tsx & .css
  - 静态/类型检查：pnpm typecheck && pnpm build 通过，零错误。
- 当前运行状态：Electron 实例在桌面常驻运行中（Wayland 环境），正在播放视频壁纸、挂载胶囊抽屉、磨砂状态栏（含进度条）与右侧精密机械齿轮机芯组。
- 下一步任务：M0-2 下半阶段 —— 主工作区居中液态玻璃面板 GlassPanel（多页面切换：当前播放/黑胶、歌单列表、歌词面板、设置面板）或状态栏音量/歌曲元数据展示。
