...
> 更新时间: 2026-09-11
> 当前阶段: M2-2 (悬浮胶囊歌单抽屉完备，双级钻取画册流+MPD歌单协议全链路闭环，460px 宽阔视窗，零报错稳定常驻)
> 最新进展: 悬浮胶囊第四按键重构为歌单 (PlaylistDrawer)，双级钻取画册流排版（歌单总览层 + 单曲详情层），MPD 原生歌单指令全套打通，创建/重命名/删除/添加/移出/播放/入队闭环，协议安全加固，详见 §2.15
...
- **M2-2 阶段: 悬浮胶囊第四按键"歌单"管理 (`PlaylistDrawer`) [已完成]**:
  - 悬浮胶囊第 4 个图标重构为歌单，纯线条制表风矢量图标，460px 展开视窗加载 PlaylistDrawer；
  - 双级钻取画册流：歌单总览层（卡片封面/歌单名/曲目数/时长/一键播放/重命名/删除）⟷ 歌单详情层（Hero 头部/全部播放/全部入队/添加单曲/单曲列表/二态开关/移出歌单）；
  - MPD 原生歌单指令全链路打通（listplaylists/listplaylistinfo/playlistadd/playlistdelete/save/rm/rename），协议安全加固。
