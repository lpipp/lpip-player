...
#### 遗留未验事项
- 方案 A（仅不干预 `<audio>`，未改编码）曾残留约 **3.2 秒响应滞后**（先听到旧位置声音再切过去）。
  改 PCM 后**未做专项复测**，不可声称已解决。
- 上述静音归零是「不碰 `<audio>`」+「PCM 编码」的**共同结果**，无法单独归因于代码变更。
- **方案 B / C 待后续单独开会话决策**（见 §5 末尾）。
...
### 待决策: 音频响应滞后的彻底消除方案（用户已定"留待后续单独开会话"）

背景见 §4.8：现行实现真实静音已归零，但仍可能残留约 3.2s **响应滞后**（先听到旧位置声音再切过去，
非静音）。彻底消除滞后需动架构，三个方案的取舍如下 —— **动手前必须先请用户拍板**：

| 方案 | 做法 | 收益 | 代价 |
|---|---|---|---|
| **A（已落地）** | 前端不干预 `<audio>` | 真实静音归零；风险最低，仅改 `App.tsx` | 残留响应滞后未彻底消除 |
| **B** | mpd.conf 增加本地 PipeWire/ALSA 输出，MPD 自己出声，前端不再拉流 | 滞后降至 MPD 自身输出延迟（约 100ms），三 bug 彻底消失 | **前端拿不到音频数据，M2-1「WebAudio FFT 驱动机芯/水波共振」将无法实现（Model B 的核心目的落空）**；需改 mpd.conf |
| **C** | 抛弃 `<audio>`，自建 `fetch` 流式读取 + WebAudio 播放，缓冲垫可压到约 200ms | 既无静音也无明显滞后，且原生拿到 FFT 数据（直接成就 M2-1） | 工程量与风险显著高于 A/B，属重构而非修 bug，可能需多个会话 |

- **用户已选定 C 的解码路径**：若将来实施 C，走 **PCM/wave 直送**（已于 2026-09-09 落地该编码）——
  前端只需解析一次 WAV 头，之后是纯 Int16 PCM 直送 WebAudio，**无需 Ogg 分页解析、无需 vorbis 解码**，
  代码量与出错面最小，且全程无损。
- 已否决的解码路径：保持 vorbis 用 WebCodecs 解码（需自行处理 Ogg 分页 + vorbis 包重组 + 三段头，
  复杂度高且音质仍有损）；改用 flac 编码（Chromium 对 FLAC 流式解码支持不稳，
  可能需引入解码库，与「零第三方依赖」纪律冲突）。
