...
```
3bfda2c feat(ui): 锁定状态栏当前曲目信息卡片为 140px 宽度并平滑省略截断，杜绝控制按键移位
dd301c2 feat(ui): 将播放模式切换按键移至播放控制组右侧紧随下一曲排布
ec42e48 docs: 更新 歌曲信息移至状态栏封面右侧与控制紧随排布 全量文档
b929bfe feat(ui): 将当前播放歌曲信息移至状态栏封面右侧，播放控制紧随其后
a4875cf fix(ui): 彻底移除背景中漂浮的多面体晶体几何图形与动效
5880e19 fix(ui): 修复歌词滚轮滑动反向滞后与动量积压Bug
2287163 docs: 更新 歌词圆弧进度条与底栏极简化 项目全量文档
3cb362b feat(ui): 将歌词旁虚线弧改为圆弧进度条并移除底部旧进度条
7b96a20 feat(ui): 在所有滑条两侧增加减号与加号微调按钮并支持长按连续步进
917505f docs: 更新 齿轮与歌词同心模块化及XY轴设置调节 全量文档
33160c0 feat(ui): 将左侧齿轮与歌词合并为同心模块并支持设置项调节XY轴
687920c feat(ui): 彻底删除底部状态栏音量调节控件与相关业务逻辑
cb66f45 docs: 更新 非交互读数去框转纯文字 项目全量文档
e009ee9 fix(ui): 将艺人数量、各类滑动条数值与统计播放次数等非交互读数去除外部边框改为纯文字
797b6ef docs: 更新 偏好设置副顶栏与分类徽标移除 项目全量文档
04ef167 fix(ui): 移除偏好设置一级总览副顶栏与二级导航分类徽标
d34177e docs: 更新 偏好设置分组卡片徽标移除 项目全量文档
b909301 fix(ui): 移除偏好设置各分组卡片标题栏冗余的徽标提示
00a8013 docs: 更新 移除无意义提示框 项目全量文档
